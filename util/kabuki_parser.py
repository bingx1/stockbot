...
if __name__ == '__main__':
    ...
    item = KabukiParser.parse_html(soup)
    print(item)
    req = requests.get('https://store.kabukistrength.net/collections/cosmetic-blemish/products/transformer-bar-blemish')
    soup = BeautifulSoup(req.text, 'html.parser')
    item = KabukiParser.parse_html(soup)
    print(item)
    req = requests.get('https://store.kabukistrength.net/collections/cosmetic-blemish/products/the-kadillac-bar-blemish')
    soup = BeautifulSoup(req.text, 'html.parser')
    item = KabukiParser.parse_html(soup)
    print(item)

    req = requests.get('https://store.kabukistrength.net/collections/cosmetic-blemish/products/copy-of-the-trap-bar')
    soup = BeautifulSoup(req.text, 'html.parser')
    item = KabukiParser.parse_html(soup)
    print(item)